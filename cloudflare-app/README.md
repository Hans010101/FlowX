...
工作台前台按「选题、稿库·发布、设置」组织，生产与审核作为后台自动环节运行。百度、今日头条、抖音、微博、知乎、B站、36氪、澎湃由 Worker 优先直抓，并以可配置的 DailyHotApi 回退；支持 Tavily → 博查搜索回退、DeepSeek 写稿、Pexels 配图、规则 + AI 质检、自动二次修改与复检，以及单轮最多 20 篇受控并发生成。正文净字数强制控制在 800–1500 字，首段为 100 字以内导语，末段为行业专家点评；每篇稿件同时提炼 3–6 处关键结论、数字或核心信息并加粗。稿库预览、富文本复制与平台同步均保留这些结构和重点样式。写稿成功或手动删除的热点会按账号移出选题池。
